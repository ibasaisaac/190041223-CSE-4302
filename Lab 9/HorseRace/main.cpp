...
{           //redraws horse in its new position
...
{
    for(int j=0; j<total_horses; j++) //delete each horse object
        delete hArray[j];
}
//---------------------------------------------------------------
...
void track::run()
{
    while( !kbhit() ) //run as long as user does not press a key
    {
        elapsed_time += 1.75; //update time
//update each horse
        for(int j=0; j<total_horses; j++)
            hArray[j]->display_horse(elapsed_time); //redraw each horse
        wait(500); //loop waits for 500 milliseconds
    }
...
//get data from user
    ...
    cin >> total;
    track theTrack(length, total); //create the track object
